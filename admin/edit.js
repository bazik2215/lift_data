...
let miniMap = null, miniMapInitialized = false, isMapVisible = false;
...
function previewCoordinates() {
    ...
    if (!coordsStr) { showToast('❌ Введите координаты'); return; }
...
function initMiniMap(lat, lon) {
    if (typeof ymaps === 'undefined') { showToast('❌ Карты не загрузились'); return; }
    ymaps.ready(() => {
        miniMap = new ymaps.Map('miniMap', { center: [lat, lon], zoom: 17, controls: ['zoomControl'] });
        miniMap.geoObjects.add(new ymaps.Placemark([lat, lon], {}, { preset: 'islands#blueHomeIcon' }));
...
function renderEntrances() {
    ...
    window.entrancesData.forEach((entrance, idx) => {
        ...
        div.innerHTML = `
            <div class="subgroup-header"><strong>🚪 Подъезд ${idx + 1}</strong><button class="action-icon" onclick="removeEntrance(${idx})">🗑️</button></div>
            <div class="form-field"><label>Название подъезда</label><input type="text" id="entrance_name_${idx}" value="${escapeHtml(entrance.name || '')}"></div>
...
function renderLifts(entranceIdx) {
    ...
    lifts.forEach((lift, liftIdx) => {
        ...
        liftDiv.innerHTML = `
            <div class="lift-header"><strong>🛗 Лифт ${liftIdx + 1}</strong><button class="action-icon" onclick="removeLift(${entranceIdx}, ${liftIdx})">🗑️</button></div>
            ...
            <details style="margin-top:12px;"><summary>📜 Предыдущий лифт (до замены)</summary>
...
function addEntrance() { saveCurrentEntranceData(); window.entrancesData.push({ name: '', lifts: [] }); renderEntrances(); }
function removeEntrance(idx) { saveCurrentEntranceData(); window.entrancesData.splice(idx, 1); renderEntrances(); }
function addLift(entranceIdx) { saveCurrentEntranceData(); if (!window[`liftsData_${entranceIdx}`]) window[`liftsData_${entranceIdx}`] = []; window[`liftsData_${entranceIdx}`].push({}); renderLifts(entranceIdx); }
function removeLift(entranceIdx, liftIdx) { saveCurrentEntranceData(); window[`liftsData_${entranceIdx}`].splice(liftIdx, 1); renderLifts(entranceIdx); }
function addProgram() { saveCurrentProgramsData(); window.programsData.push({ year: '', description: '' }); renderPrograms(); }
function removeProgram(idx) { saveCurrentProgramsData(); window.programsData.splice(idx, 1); renderPrograms(); }
function addShortTerm() { saveCurrentShortTermData(); window.shortTermData.push({ type: '', contractor: '', period: '' }); renderShortTerm(); }
function removeShortTerm(idx) { saveCurrentShortTermData(); window.shortTermData.splice(idx, 1); renderShortTerm(); }
...
function renderPrograms() {
    ...
    window.programsData.forEach((prog, idx) => {
        const div = document.createElement('div');
        div.className = 'subgroup';
        div.innerHTML = `<div class="subgroup-header"><strong>📅 Программа ${idx+1}</strong><button class="action-icon" onclick="removeProgram(${idx})">🗑️</button></div>
...
function renderShortTerm() {
    ...
    window.shortTermData.forEach((term, idx) => {
        const div = document.createElement('div');
        div.className = 'subgroup';
        div.innerHTML = `<div class="subgroup-header"><strong>⚡ План ${idx+1}</strong><button class="action-icon" onclick="removeShortTerm(${idx})">🗑️</button></div>
...
async function saveHouse() {
    ...
    if (!address) { showToast('❌ Введите адрес дома'); return; }
    ...
    if (coordsStr) {
        ...
        else { showToast('❌ Неверный формат координат'); return; }
    }
    
    const entrances = [];
    for (let i = 0; i < window.entrancesData.length; i++) {
        entrances.push({ name: window.entrancesData[i].name, lifts: window[`liftsData_${i}`] || [] });
    }
